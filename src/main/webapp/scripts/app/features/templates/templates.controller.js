...
//        	   $state.go($state.current, {}, { "reload": true })
   ...
   $scope.templates2 = resolveTemplates.items;
	...
	$scope.delTemplate = function(template){
		var req = {
				 method: "DELETE",
				 url: "http://localhost:8080/spade/api/demo/controllers/" + template.labels.controller
		};
		console.log(template)
...
.factory('TemplateService2', function($http) {
	return {
		getPods : function() {
			var promise = $http.get("http://localhost:8080/spade/api/stack_templates")
...
// 		$scope.showCustomToast();
 		 	$http.post("http://localhost:8080/spade/api/demo/stacks", $scope.payload)
     		 	.success(function(data){
