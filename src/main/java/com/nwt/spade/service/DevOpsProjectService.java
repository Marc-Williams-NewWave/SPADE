...
public class DevOpsProjectService {
	...
	public List<CheckinChart> findAllCheckins(String name) {
		...
		String url = "https://api.github.com/repos/" + project.getGitUrl()
				+ "/commits";

		ResponseEntity<Checkin[]> response = restTemplate.exchange(url,
				HttpMethod.GET, request, Checkin[].class);
		Checkin[] checkins = response.getBody();
		Map<String, Integer> occurrences = new HashMap<String, Integer>();

		for (Checkin checkin : checkins) {

			String date = checkin.getCommit().getCommitter().getDate()
					.substring(0, 10);
	...
	public Contributor[] findAllcontributors(String name) {
		...
		// BuildInfo
		// builds=restTemplate1.getForObject("http://192.168.1.10:8080/job/EPPE-CI/api/json",
		// BuildInfo.class);
		//
		...
		String url = "https://api.github.com/repos/" + project.getGitUrl()
				+ "/stats/contributors";
		ResponseEntity<Contributor[]> response = restTemplate.exchange(url,
				HttpMethod.GET, request, Contributor[].class);
	...
	public List<Builds> insertBuildInformation(String name) {
		...
		BuildInfo allbuilds = restTemplate.getForObject(
				"http://192.168.1.10:8080/job/EPPE-CI/api/json",
				BuildInfo.class);
		...
		if (project.getLatestBuildNumber() == 0) {
			if (latestBuild.getNumber() > 0) {
				for (BuildAll buildAll : allbuilds.getBuilds()) {
					...
					Builds latestBuild1 = restTemplate.getForObject(
							buildAll.getUrl() + "api/json", Builds.class);
			...
			for (int i = project.getLatestBuildNumber(); i < latestBuild
					.getNumber(); i++) {
				Builds build = new Builds();
				String url1 = buildurl + ":8080/job/EPPE-CI/" + i + "/api/json";

				Builds latestBuild1 = restTemplate.getForObject(url1,
						Builds.class);
				build.setDuration(latestBuild1.getDuration());
				build.setNumber(latestBuild1.getNumber());
				build.setResult(latestBuild1.getResult());
	...
	public List<IssueChart> getIssues(String name) {
		...
		ResponseEntity<Severity> response = restTemplate
				.exchange(
						"http://192.168.1.10:9000/api/issues/search?componentRoots={project}&severities={severities}&statuses={statuses}",
						HttpMethod.GET, request, Severity.class, projectName,
						"MINOR", "OPEN");
		...
		ResponseEntity<Severity> response1 = restTemplate
				.exchange(
						"http://192.168.1.10:9000/api/issues/search?componentRoots={project}&severities={severities}&statuses={statuses}",
						HttpMethod.GET, request, Severity.class, projectName,
						"MAJOR", "OPEN");
		...
		ResponseEntity<Severity> response2 = restTemplate
				.exchange(
						"http://192.168.1.10:9000/api/issues/search?componentRoots={project}&severities={severities}&statuses={statuses}",
						HttpMethod.GET, request, Severity.class, projectName,
						"CRITICAL", "OPEN");
		...
		ResponseEntity<Severity> response3 = restTemplate
				.exchange(
						"http://192.168.1.10:9000/api/issues/search?componentRoots={project}&severities={severities}&statuses={statuses}",
						HttpMethod.GET, request, Severity.class, projectName,
						"BLOCKER", "OPEN");
		...
		ResponseEntity<Severity> response4 = restTemplate
				.exchange(
						"http://192.168.1.10:9000/api/issues/search?componentRoots={project}&severities={severities}&statuses={statuses}",
						HttpMethod.GET, request, Severity.class, projectName,
						"INFO", "OPEN");
