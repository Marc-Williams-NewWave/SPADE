...
//	      proj.size();
   ...
   public List<Builds> insertBuildInformation(String name)
   {
	  ...
	  else
	  {
	   ...
	   BuildInfo allbuilds=restTemplate.getForObject("http://192.168.1.10:8080/job/EPPE-CI/api/json", BuildInfo.class);
	 ...
	 if(project.getLatestBuildNumber()==0)
	 {
	 ...
	 else if(project.getLatestBuildNumber()<latestBuild.getNumber())
	  {
		  ...
		  {
			  Builds build = new Builds();
			  String url1=buildurl+":8080/job/EPPE-CI/"+i+"/api/json";
...
public List<IssueChart> getIssues(String name) {
    ...
    ResponseEntity<Severity> response = restTemplate.exchange("http://192.168.1.10:9000/api/issues/search?projects={project}&severities={severities}&statuses={statuses}", HttpMethod.GET, request, Severity.class,"cmsEppe","MINOR","OPEN");
    Severity minor = response.getBody();
    chart = new IssueChart();
    chart.setValue(minor.getPaging().getTotal());
    chart.setLabel("MINOR");
    issues.add(chart);
    
    ResponseEntity<Severity> response1 = restTemplate.exchange("http://192.168.1.10:9000/api/issues/search?projects={project}&severities={severities}&statuses={statuses}", HttpMethod.GET, request, Severity.class,"cmsEppe","MAJOR","OPEN");
    Severity major = response1.getBody();
    chart = new IssueChart();
    chart.setValue(major.getPaging().getTotal());
    chart.setLabel("MAJOR");
    issues.add(chart);
    
    ResponseEntity<Severity> response2 = restTemplate.exchange("http://192.168.1.10:9000/api/issues/search?projects={project}&severities={severities}&statuses={statuses}", HttpMethod.GET, request, Severity.class,"cmsEppe","CRITICAL","OPEN");
    Severity critical = response2.getBody();
    chart = new IssueChart();
    chart.setValue(critical.getPaging().getTotal());
    chart.setLabel("CRTICAL");
    issues.add(chart);
    
    
    ResponseEntity<Severity> response3 = restTemplate.exchange("http://192.168.1.10:9000/api/issues/search?projects={project}&severities={severities}&statuses={statuses}", HttpMethod.GET, request, Severity.class,"cmsEppe","BLOCKER","OPEN");
    Severity blocker = response3.getBody();
    chart = new IssueChart();
    chart.setValue(blocker.getPaging().getTotal());
    chart.setLabel("BLOCKER");
    issues.add(chart);
    
    ResponseEntity<Severity> response4 = restTemplate.exchange("http://192.168.1.10:9000/api/issues/search?projects={project}&severities={severities}&statuses={statuses}", HttpMethod.GET, request, Severity.class,"cmsEppe","INFO","OPEN");
